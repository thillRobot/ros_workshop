...
In Part I you will setup a _catkin workspace_ as the working directory for your ROS system. Catkin is the program that manages your custom nodes and compiles your .cpp source code into executable programs. (_[Catkin](http://wiki.ros.org/catkin) is the low-level build system and infrastructure for ROS_. [cmake](https://cmake.org/) manages compiling.)
...
### Bonus Execrcise: Download the Python source code for keyboard package with `git`

Nodes written in python are also stored in the source directory of a package in the catkin workspace. Python nodes do not require compiling before running. (Ubuntu 20.04 ships with Python 3.8.10, `python3 --version`)
 

Clone the package `teleop-twist-keyboard` into a catkin workspace source folder.

```
mkdir -p ~/catkin_ws/src
```

Install `git`, it may already be installed. 
...
Download the `teleop-twist-keyboard` package using `git` from `https://github.com/ros-teleop/teleop_twist_keyboard`.  
